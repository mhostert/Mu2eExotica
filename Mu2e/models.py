...
class MuonCaptureModelI:
    def __init__(self, maprime, m2, m1, Gmup, alphaD=0.1, epsilon=1e-4) -> None:
        ...
        alphaQED = r"\alpha" if self.alphaD == const.alphaQED else rf"{self.alphaD:.1f}"
        self.text_descriptor = rf"\noindent$\alpha_D = {alphaQED}$\\$G_{{\mu p}} = ({self.tex_scale}$ \,TeV$)^{{-2}}$\\$m_2 = {self.m2*1e3:.0f}$ MeV\\$m_1 = {self.m1*1e3:.0f}$ MeV\\$m_{{A^\prime}} = {self.maprime*1e3:.0f}$ MeV"
